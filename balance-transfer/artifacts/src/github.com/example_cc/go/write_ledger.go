...
// ============================================================================================================================
// Init order - create a new transaction order, store into chaincode state
//
// Shows off building a key's JSON value manually
//

//  0              1            2           3            4                5                6                     
//   id          FromCompany ,  ToCompany,    Assetname,    AsseType,  OrderedQuantity,    OrderedDate,

// "t3535757"       Audi,         bmw  ,     audia6  ,      audicar,        10      ,         26.07.18

// ============================================================================================================================

func init_order (stub shim.ChaincodeStubInterface, args []string) (pb.Response) {
	var err error
	fmt.Println("starting order_transation")
	...
	assetname := args[3]
        ...
        orderquantity := args[5]
        orderdate  := args[6]        
	
	//check if order id already exists
	trans, err := get_order(stub, id)
	if err == nil {
		fmt.Println("This transcation is already exists - " + id)
		fmt.Println(trans)
		return shim.Error("This transcation is already exists - " + id)  //all stop a marble by this id exists
	}

	//build the order json string manually
	str := `{
		"docType":"order", 
		"id": "` + id + `", 
		"assetname": "` + assetname + `", 
	...
	err = stub.PutState(id, []byte(str))                         //store order with id as key
	if err != nil {
		return shim.Error(err.Error())
	}

	fmt.Println("- end init_transcation")
	return shim.Success(nil)
}
